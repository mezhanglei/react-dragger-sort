...
import { DropEffect, DndParams, DndHandle, EventType, DndSortableProps } from "./types";
...
class DndSortable extends React.Component<DndSortableProps> {
  ...
  constructor(props: DndSortableProps) {
    super(props);
    this.parentElRef = this.props.forwardedRef || React.createRef();
    this.dndManager = this.props.dndManager || new DndManager();
    this.state = {
    };
  }
  ...
  initManagerData = (parentEl?: HTMLElement | null) => {
    ...
    for (let i = 0; i < len; i++) {
      ...
      this.dndManager.setDragItemsMap(childNode, {
        node: childNode,
        id: dataId,
        group: {
          node: parentEl,
    ...
    // 初始化可拖放元素
    this.dndManager.setDropItemsMap(parentEl, {
      group: {
        node: parentEl,
        ...props
      }
  ...
  handleMoveStart = (e: any, currentTarget: HTMLElement) => {
    ...
    const dragItem = this.dndManager.getDragItem(currentTarget);
    if (dragItem) {
      const params = {
        e,
        from: {
  ...
  onStart: EventHandler = (e) => {
    ...
    const dragItem = this.dndManager.getDragItem(currentTarget);
    const targetItem = this.dndManager.getDragItem(target);
  ...
  onEndHandle = (e: EventType) => {
    ...
    const parentEl = this.parentElRef?.current;
    const dragItem = this.dndManager.getDragItem(dragged);
    const toItem = over && this.dndManager.getDragItem(over);
    const toGroup = over && this.dndManager.getDropItem(over);
    const to = (toItem || toGroup);
    const dropGroup = to?.group;
    const toIndex = getChildrenIndex(cloneDragged, [dragged]);
    const fromIndex = getChildrenIndex(dragged, [cloneDragged]);
    const params = {
      e,
      from: {
        ...dragItem,
  ...
  moveHandle: EventHandler = (e) => {
    ...
    const target = this.dndManager.findOver(e, dragged, isOverSelf);
    const dragItem = this.dndManager.getDragItem(dragged);
    // 触发目标
    if (dragItem) {
      const toItem = target && this.dndManager.getDragItem(target);
      const toGroup = target && this.dndManager.getDropItem(target);
      const to = (toItem || toGroup);
      const dropGroup = to?.group;
      const params = {
        e,
  ...
  render() {
    ...
    const cls = classNames('dnd-sortable-parent', className);
...
export default function BuildDndSortable() {
  ...
  return React.forwardRef((props: DndSortableProps, ref) => {
    return (
      <DndSortable forwardedRef={ref} {...props} dndManager={dndManager} />
    );
  });
}
